...
std::vector<unsigned char> EncoderSystem::encode(std::vector<unsigned short> data)
{
    std::vector<unsigned char> encoded(480 * CHANNELS);
    _nbBytes = opus_encode(
        _encode,
...
std::vector<unsigned short> EncoderSystem::decode(std::vector<unsigned char> encoded)
{
    std::vector<unsigned short> decoded(480 * CHANNELS);
    _nbBytes = opus_decode(
        _decode,
        encoded.data(),
        sizeof(encoded),
        ...
        0);
    if (_nbBytes < 0)
        std::cerr << "Error: opus_decode failed" << std::endl;
    return decoded;
